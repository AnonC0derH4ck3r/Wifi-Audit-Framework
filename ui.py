...
class UI:
    ...
    def show_menu() -> int:
        menu_rows = [
            ...
            ["7", "Hidden SSID Decloak",       "Reveal SSID of hidden APs via client frames"],
        ...
        while True:
            try:
                choice = input(f"{UI.BOLD}  ❯ Enter choice [0-7]: {UI.RESET}").strip()
                if choice in {"1", "2", "3", "4", "5", "6", "7", "0"}:
                    return int(choice)
                UI.warn(f"Invalid option '{choice}'. Choose 0–7.")
