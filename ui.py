...
class UI:
    ...
    def select_hidden_ap_from_results(results: list) -> tuple:
        ...
        """
        hidden = [r for r in results if str(r[2]).strip() in ("<Hidden>", "<hidden>", "<Malformed>", "<malformed>", "", "<Hidden>", None) or str(r[2]).strip() == ""]
        # Also treat any SSID that looks like placeholder as hidden
        hidden = [r for r in results if str(r[2]).strip().lower() in ("<hidden>", "<malformed>", "") or not str(r[2]).strip() or str(r[2]).strip().startswith("<")]
        if not hidden:
