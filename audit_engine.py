...
class WirelessAuditEngine:
    ...
    def __init__(self, interface: str):
        ...
        self.client_status  = {}
        # For timeout-based disconnect: last data-activity timestamp per client (monotonic)
        # Distinguish Idle vs Disconnected: data = active, any frame = present
        self.client_last_seen: dict = {}  # last DATA frame (active internet use)
        self.client_last_any: dict = {}   # last ANY frame from client (probe/null/assoc/data) — presence
        self.client_timeout = 8   # seconds — no data for this long → consider idle/disconnected check
        self.client_idle_timeout = 30  # seconds — no ANY frame for this long → truly Disconnected; else Idle
        ...
        self.decloaked_results = []  # list of [bssid, ssid, client_mac, timestamp, frame_type]
    ...
    def _touch_client(self, mac: str):
        """Refresh last-seen timestamp; if it was Disconnected/Idle, flip to Connected."""
        now = time.monotonic()
        self.client_last_seen[mac] = now
        self.client_last_any[mac] = now
        if self.client_status.get(mac) in ("Disconnected", "Idle"):
            self.client_status[mac] = "Connected"
            self._update_row_status(mac)
            self.render_client_table()

    def _touch_any(self, mac: str):
        """Refresh presence timestamp (any frame) — used for Idle vs Disconnected distinction."""
        self.client_last_any[mac] = time.monotonic()
        # If it was Disconnected but we now see any frame (probe/null), mark Idle, not Connected
        # Connected requires data; Idle is for presence without data
        if self.client_status.get(mac) == "Disconnected":
            # We saw a non-data frame, so client is still present but idle
            # Check if we have ever seen data from this client — if yes, Idle is appropriate
            # Otherwise, it might be a new client probing, but we handle via _mark_connected for assoc
            # For generic any, mark Idle if we have seen it before
            if mac in self.seen_clients:
                self.client_status[mac] = "Idle"
                self._update_row_status(mac)
                self.render_client_table()
    ...
    def _client_timeout_loop(self):
        """Three-state: Connected (data <8s), Idle (data >8s but any <30s), Disconnected (any >30s or Deauth)."""
        while not self._client_monitor_stop.is_set():
            now = time.monotonic()
            for mac in list(self.seen_clients):
                status = self.client_status.get(mac)
                last_data = self.client_last_seen.get(mac, 0)
                last_any = self.client_last_any.get(mac, last_data)
                # If never seen data, use last_any for both
                if last_data == 0:
                    last_data = last_any
                if status == "Connected" and now - last_data > self.client_timeout:
                    # No data for 8s — check if we still see any frames (idle) or truly gone
                    if now - last_any <= self.client_idle_timeout:
                        self.client_status[mac] = "Idle"
                    else:
                        self.client_status[mac] = "Disconnected"
                    self._update_row_status(mac)
                    self.render_client_table()
                elif status == "Idle" and now - last_any > self.client_idle_timeout:
                    # Idle but no any frame for 30s → truly Disconnected
                    self.client_status[mac] = "Disconnected"
                    self._update_row_status(mac)
                    self.render_client_table()
                elif status == "Idle" and now - last_data <= self.client_timeout:
                    # Idle but data resumed → Connected (should have been handled by _touch_client, but catch here)
                    self.client_status[mac] = "Connected"
                    self._update_row_status(mac)
                    self.render_client_table()
            time.sleep(1)
    ...
    def hidden_decloak(self, pkt, target_bssid: str):
        ...
        for row in self.results:
            if row[0].lower() == target_bssid and str(row[2]).strip().lower() in ("<hidden>", "<malformed>", "", "<hidden>"):
                row[2] = ssid
                break
        ...
        self.decloaked_results.append([target_bssid, ssid, client_mac, vendor, frame_type, now])
    ...
    def render_decloaked_table(self, target_bssid: str = None):
        ...
        display_rows = []
        for r in rows:
            bssid, ssid, client_mac, vendor, ftype, ts = r
            display_rows.append([
        ...
        print(tabulate(display_rows, headers=["BSSID (Hidden AP)", "Decloaked SSID", "Client MAC", "Client Vendor", "Frame", "Time"], tablefmt="pretty"))
    ...
    def data_frames(self, pkt, s_bssid, iface, channel):
        ...
        # ── Presence heartbeat for Idle vs Disconnected ───────────────────────
        # Any frame from a known client (even Probe/Null) proves it's still present.
        # We update last_any here so idle clients (no data for 8s but still probing)
        # stay as Idle instead of flipping to Disconnected until 30s of total silence.
        try:
            src = (dot11.addr2 or "").lower()
            if src and src in (m.lower() for m in self.seen_clients):
                self.client_last_any[src] = time.monotonic()
                # If it was Disconnected but we now see any frame, it's at least Idle
                if self.client_status.get(src) == "Disconnected":
                    self.client_status[src] = "Idle"
        ...
        if dot11.type == 0x02:
            ...
            # Distinguish active data (with IP) vs idle keep-alive (Null/QoS-Null, no IP)
            # Active = definitely using internet; Idle = present but not using data
            is_active = False
            try:
                # Check for IP/TCP/UDP layers — scapy dissects LLC/IP if present
                from scapy.layers.inet import IP as ScapyIP
                from scapy.layers.inet6 import IPv6 as ScapyIPv6
                if pkt.haslayer(ScapyIP) or pkt.haslayer(ScapyIPv6) or pkt.haslayer("TCP") or pkt.haslayer("UDP"):
                    is_active = True
                else:
                    # Fallback: check Raw payload length — Null Data has no IP, just 802.11 header
                    # QoS Null / Null have subtype 4/12 with no LLC; real data has LLC+IP (≥ 26 bytes)
                    if pkt.haslayer("Raw"):
                        try:
                            raw = bytes(pkt.getlayer("Raw").load) if hasattr(pkt.getlayer("Raw"), "load") else b""
                            # LLC header is 8 bytes (AA AA 03 00 00 00 08 00 for IP); need at least that + IP
                            if len(raw) > 16:
                                is_active = True
                        except Exception:
                            is_active = False
                    # Also check Dot11 subtype: 0x08 = QoS Data, 0x20 = Null, 0x24 = QoS Null — these are idle
                    # Data subtypes 0x00/0x08 with ToDS=1 and non-zero payload are active
                    # If subtype is Null (4) or QoS Null (12/0x0C), treat as idle even if Raw exists
                    try:
                        if dot11.subtype in (4, 12):  # Null, QoS-Null
                            is_active = False
                    except Exception:
                        pass
            except Exception:
                is_active = True  # fallback to active if check fails

            # New client — add row for the first time
            if client_mac not in self.seen_clients:
                ap_vendor     = self._lookup_oui(ap_mac)
                client_vendor = self._lookup_oui(client_mac)
                # If first frame is Null/idle, mark Idle, else Connected
                init_status = "Connected" if is_active else "Idle"
                self.client_status[client_mac] = init_status
                now_ts = time.monotonic()
                self.client_last_seen[client_mac] = now_ts if is_active else self.client_last_seen.get(client_mac, 0)
                self.client_last_any[client_mac] = now_ts
                # For Idle, last_data stays 0 so monitor will keep it Idle until active data arrives
                if not is_active:
                    # Ensure last_seen for data is old so it stays Idle
                    if client_mac not in self.client_last_seen or self.client_last_seen[client_mac] == 0:
                        self.client_last_seen[client_mac] = now_ts - (self.client_timeout + 1)
                row = [ap_mac, ap_vendor, client_mac, client_vendor, init_status]
                self.client_results.append(row)
                self.seen_clients.add(client_mac)
            else:
                # Known client — update timestamps
                now_ts = time.monotonic()
                self.client_last_any[client_mac] = now_ts
                if is_active:
                    ...
                    # Idle keep-alive (Null): don't refresh last_data, just last_any
                    # If it was Disconnected and we see any frame, it becomes Idle (handled by top any-frame check too)
                    if self.client_status.get(client_mac) == "Disconnected":
                        self.client_status[client_mac] = "Idle"
                        self._update_row_status(client_mac)
                    elif self.client_status.get(client_mac) == "Connected":
                        # Stay Connected until timeout flips to Idle — don't flip immediately
                        pass
